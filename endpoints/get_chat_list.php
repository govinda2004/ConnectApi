 ...
 * Returns: conversations with last message + unread count
 */
...
foreach ($conversations as $conv) {
    ...
    // Get user info
    $stmt2 = $db->prepare('
        SELECT u.id, u.name, pr.profile_image, pr.headline
        FROM users u LEFT JOIN profiles pr ON u.id = pr.user_id
        WHERE u.id = ?
    ');
    ...
    $stmt3 = $db->prepare('SELECT COUNT(*) FROM messages WHERE sender_id = ? AND receiver_id = ? AND read_at IS NULL AND is_deleted = 0');
    $stmt3->execute([$otherId, $userId]);
    $unread = (int)$stmt3->fetchColumn();
